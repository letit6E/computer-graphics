...
public:
    ...
    WaveletTree(const std::vector<double>& vec) {
        n = vec.size();
        if (vec.empty()) {
            lo = DBL_MAX;
            hi = DBL_MIN;
        ...
        prefix_sum = std::vector<size_t>(vec.size() + 1, 0);
    ...
    WaveletTree(const std::vector<std::pair<double, size_t>>& vec) {
        n = vec.size();
        lo = 1e9;
        hi = -1e9;
        if (vec.empty()) {
        ...
        prefix_cnt.resize(vec.size() + 1);
        prefix_sum.resize(vec.size() + 1);
        prefix_sum[0] = prefix_cnt[0] = 0;

        if (hi == lo) {
            for (size_t i = 1; i <= vec.size(); ++i) {
                prefix_cnt[i] = i;
                prefix_sum[i] = prefix_sum[i - 1] + vec[i - 1].second;
            }
            return;
        }
    ...
    size_t lte_sum(int l, int r, double k) {
        if (l > r || k < lo) return 0;
        l = std::max(l, 0);
        r = std::min(r, static_cast<int>(n) - 1);
        if (hi <= k) return prefix_sum[r + 1] - prefix_sum[l];

        int left_cnt = static_cast<int>(prefix_cnt[l]), right_cnt = static_cast<int>(prefix_cnt[r + 1]);
        return left->lte_sum(left_cnt, right_cnt - 1, k) + right->lte_sum(l - left_cnt, r - right_cnt, k);
    }
